...
- `#系統通知`：只記錄 Hermes / wiki runtime 狀態，不放知識內容正文。
...
Hermes 未來推播到 Discord 時，系統狀態與知識內容要分流，避免 `#系統通知` 變成所有內容的垃圾桶。
...
`#系統通知` 只放 runtime health，例如 sync / compile / Discord poll / Joplin writeback 的成功、失敗與 safe error code。內容摘要、draft、專案 follow-up 與主題整理結果應回到對應內容頻道。
