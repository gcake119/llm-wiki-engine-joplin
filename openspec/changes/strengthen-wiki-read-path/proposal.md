...
這個問題屬於成熟的 agent-native knowledge system / retrieval-as-reasoning 強化：先穩定 ingestion，再提供 search / read / links / sufficiency 的可組合工具，讓 Hermes 自己決定下一步。不要把它做成傳統 RAG hidden retriever，也不要把 page synthesis、consolidation、Error Book 這類內部治理概念暴露成一堆 CLI 指令。

## What Changes

- 在同一個 change 內分期強化 LLM-Wiki 最小指令集：
  ...
  - Phase 7：定義 source-backed page artifact 只作為 `wiki compile` 的內部輸出，不新增 `wiki synthesize` command。
  - Phase 8：把 Telegram / Discord / feedback / consolidation 都收斂到 `wiki draft ...`，不新增 `wiki consolidate` command。
  - Phase 9：讓 `wiki approve <draft-id>` 成為唯一 Joplin writeback 入口，並要求 provenance、conflict check 與 explicit approval。
...
- 不新增 `wiki ask`、`wiki synthesize`、`wiki consolidate`、`wiki error-book` 這些額外 top-level commands；ask 由 Hermes 做，synthesis/consolidation/error book 先是內部 artifact / draft behavior。
- 不讓 `wiki query` / `wiki read` / `wiki links` 在 foreground 呼叫 Joplin、embedding service、vector DB 或外部 retrieval API。
